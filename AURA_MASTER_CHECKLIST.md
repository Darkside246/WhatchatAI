...
**Deliberately not done in this pass**: removing the dead `trialPolicy.ts`/`canUseProductAccount()` code - flagged, not deleted, since confirming it's truly unreachable everywhere (not just via grep) deserves its own careful look rather than a drive-by deletion in an already-large session.
...
[ ] 13      - Conversational memory - substantially covered by 04/06/10/14-24's work; not yet formally re-verified as its own section
