...
router.post('/developer/payment-proofs/:proofId/review', requireAuth, requireDeveloper, async (req, res) => {
  ...
  try {
    ...
    const proofResult = await client.query<{ id: string; payment_attempt_id: string; product_account_id: string; status: string; amount_minor: string; currency: string; subscription_id: string | null }>(
      `SELECT p.id, p.payment_attempt_id, p.product_account_id, p.status, pa.amount_minor, pa.currency, pa.subscription_id
       FROM payment_proof_submissions p JOIN payment_attempts pa ON pa.id = p.payment_attempt_id
       WHERE p.id = $1 AND p.product_account_id = pa.product_account_id FOR UPDATE`, [proofId.data]);
    const proof = proofResult.rows[0];
    if (!proof) return res.status(404).json({ error: 'PAYMENT_PROOF_NOT_FOUND' });
    if (proof.status !== 'PENDING_REVIEW') return res.status(409).json({ error: 'PAYMENT_PROOF_ALREADY_REVIEWED' });
