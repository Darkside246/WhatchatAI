...
router.get('/developer/secrets-status', requireAuth, requireDeveloper, async (_req, res) => {
  ...
  return res.status(200).json({
    secrets: [
      ...
      { name: 'AI_PROVIDER_ALLOWLIST', configured: configured(process.env.AI_PROVIDER_ALLOWLIST) },
      { name: 'AI_PROVIDER_CONSENT', configured: configured(process.env.AI_PROVIDER_CONSENT) },
