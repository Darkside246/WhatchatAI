...
function AiProviderCard({ name, role, secretName, secrets, capabilities, note }: AiProviderCardProps) {
  const configured = secrets.some((s) => s.name === secretName && s.configured);
  return (
    <div className="space-y-3 rounded-xl border border-border-subtle bg-surface-1 p-4">
      ...
      <div className="flex items-center justify-between gap-3 rounded-lg bg-surface-2 px-3 py-2">
        <div>
          <p className="text-caption font-medium text-fg">Routing eligible</p>
          <p className="text-meta text-fg-muted">Provider policy is managed by the gateway</p>
        </div>
        <ToggleSwitch
          checked={configured}
          onChange={() => undefined}
          disabled={true}
          label={`${name} routing eligibility`}
        />
      </div>
      <p className="text-meta text-fg-secondary">
        {note ?? 'Eligible for automatic provider routing when configured. Provider enable/disable is controlled by the gateway policy.'}
      </p>
    </div>
  );
}
            ...
            <div className="space-y-4 border-t border-border-subtle px-6 pb-6 pt-4">
              <div className="rounded-xl border border-border-subtle bg-surface-2 p-4">
                ...
                <p className="mt-1.5 text-meta leading-5 text-fg-secondary">
                  Aura selects an eligible provider based on the request capabilities and configured provider policy. Tool-using requests are only sent to providers that support tool calling. Goose remains the text-only emergency fallback.
                </p>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <AiProviderCard
                  name="OpenAI"
                  role="tool-capable fallback"
                  secretName="OPENAI_API_KEY"
                  secrets={secretsStatus}
                  capabilities={['Chat', 'Tool calling', 'JSON']}
                />
                <AiProviderCard
                  name="Groq"
                  role="tool-capable fallback"
