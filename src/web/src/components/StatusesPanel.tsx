...
function StatusMedia({ status }: { status: WorkspaceStatus }) {
  if (status.statusType === 'text' || !status.media) {
    return (
      <div className="flex items-center gap-2 rounded-lg bg-surface-2 px-3 py-6 text-caption text-fg-muted">
  ...
  if (media.downloadStatus === 'pending' || media.downloadStatus === 'downloading') {
    return (
      <div className="flex items-center gap-2 rounded-lg bg-surface-2 px-3 py-6 text-caption text-fg-secondary">
  ...
  if (media.downloadStatus === 'failed') {
    return (
      <div className="flex items-center gap-2 rounded-lg bg-surface-2 px-3 py-6 text-caption text-error">
  ...
  return (
    <div className={`h-full flex-col overflow-y-auto ${className}`}>
      ...
      <div className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {statuses === null && !error && <p className="col-span-full text-body text-fg-muted">Loading real statuses…</p>}
        {statuses?.length === 0 && (
          <p className="col-span-full text-body text-fg-muted">No active statuses. Real updates will appear here as they're posted.</p>
        )}
        {statuses?.map((status) => (
          <div key={status.id} className="overflow-hidden rounded-xl border border-border-subtle bg-surface-1">
            ...
            <div className="flex items-center gap-2 p-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-body font-medium text-fg">{status.displayName}</p>
                <p className="text-meta text-fg-muted">{formatTime(status.createdAt)}</p>
              </div>
            </div>
          </div>
        ))}
