-- Adds account_deletion_requested, account_deletion_cancelled, and
-- phone_number_changed to security_audit_logs_event_type_check - see
-- accountDeletionService.ts/phoneNumberChangeService.ts. Per 927's own
...
-- union in securityAuditLogRepository.ts, copied exactly, so the two never
-- drift apart again.
