-- Adds the two password-reset audit event types.
--
-- Third time this pattern has bitten in this branch, so it is worth stating
-- plainly: event_type is guarded by a TypeScript union AND this CHECK
-- constraint, and only the union is visible while writing the code that
-- records the event. Adding it there compiles and passes every test that
-- does not actually write the row. Both of these are recorded on paths
-- where a rejected INSERT would be noticed only as a 500 during a password
-- reset - by someone who is, by definition, already locked out.
--
-- Full restatement, as 999, 1023 and 1025 did.
