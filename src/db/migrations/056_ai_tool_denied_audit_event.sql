-- AI Security Governor (extends the Phase 3 zero-trust tool guard):
-- previously a rejected tool call (unregistered tool, and now also
-- unknown tenant/actor, a SYSTEM-tier tool, or a rate-limit breach) threw
-- an error but wrote nothing to the audit trail - an operator had no way
-- to see that a denial had even happened. ai_tool_denied makes every
-- denial a real, visible row, matching the existing ai_tool_invoked
-- convention for allowed calls.
