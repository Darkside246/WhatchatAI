-- DSPy prompt optimization channel (see migration 057): every state
-- transition on an ai_agent_prompt_optimizations row is a real, visible
-- audit event, same discipline as ai_tool_invoked/ai_tool_denied - an
-- imported optimization is untrusted-input from an offline process, and an
-- approval is the one action that actually changes what a live AI agent
-- says to real customers.
