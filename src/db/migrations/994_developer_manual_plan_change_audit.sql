-- Developer Control Plane: a real manual override for a business's
-- subscription plan, for the case a real payment fell outside the
-- automated verify-then-apply webhook flow (a missed webhook, a
-- provider not wired, an out-of-band/cash payment). changePlan() itself
-- already existed (planUpgradeService.ts's verified-payment path) -
-- this is its first manual caller, and gets its own distinct audit
-- event so it's never confused with an automated, payment-verified change.
