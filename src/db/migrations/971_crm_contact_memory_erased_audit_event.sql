-- Adds crm_contact_memory_erased to security_audit_logs_event_type_check -
-- see workspaceService.ts's eraseCrmContactMemory (Section 75-91: the
-- single-subject erasure counterpart to exportCrmContactData), which
-- records this event every time a business erases one contact's
-- cross-conversation/per-conversation AI memory. Per 927/940/962's
-- established convention: this constraint is the single source of truth,
-- rewritten wholesale (not incrementally) from SecurityEventType's own
-- union, copied exactly.
