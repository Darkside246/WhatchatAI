-- Product-account billing depends on the product-account foundation in migration 906.
-- Keep this migration after 906 so fresh databases can apply the dependency chain
-- in lexical migration order. IF NOT EXISTS also makes the move safe for an
-- environment that may have applied the former 100 migration already.

CREATE TABLE IF NOT EXISTS product_account_subscriptions (
...
CREATE UNIQUE INDEX IF NOT EXISTS product_account_one_active_subscription
...
CREATE UNIQUE INDEX IF NOT EXISTS payment_attempt_provider_event_unique
  ON payment_attempts(provider, provider_event_id)
  WHERE provider_event_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS payment_attempts_account_status_idx ON payment_attempts(product_account_id, status);
...
CREATE INDEX IF NOT EXISTS payment_proof_submissions_status_idx ON payment_proof_submissions(status, created_at);
...
CREATE INDEX IF NOT EXISTS payment_audit_events_account_idx ON payment_audit_events(product_account_id, created_at DESC);
