-- Adds business_purged_by_developer to the audit event whitelist.
--
-- Same two-guard shape as 1023: a TypeScript union and this CHECK
-- constraint both have to know about an event type, and only the union is
-- visible while writing the code that records it. This one is recorded on
-- an un-caught await immediately before an irreversible purge, so without
-- the constraint entry the deletion would fail with a 500 and - correctly -
-- never run. Safe, but useless.
--
-- Full restatement, following the pattern 999 and 1023 established.
...
ALTER TABLE security_audit_logs ADD CONSTRAINT security_audit_logs_event_type_check CHECK (event_type IN (
  ...
  'subscription_trial_extended', 'password_changed', 'business_purged_by_developer',
