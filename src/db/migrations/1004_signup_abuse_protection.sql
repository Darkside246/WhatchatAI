-- Signup/attack visibility for the auth rate limiter and reCAPTCHA gate
-- added to the public trial-signup route (src/services/recaptchaService.ts,
-- src/server/index.ts's authLimiter) - both fire before any business/user
-- exists yet, so business_id is genuinely null here (migration 974's
-- platform-scope precedent), never a fabricated placeholder id.
--
-- 1002 was the last migration to touch this constraint - re-grepped
