...
export interface DeviceContext { ipAddress: string | null; userAgent: string | null; }
export interface AuthResult { user: PublicUser; business: BusinessRecord; membership: BusinessMembershipRecord; token: string; session: SessionRecord; }
...
export async function register(input: RegisterInput, device: DeviceContext): Promise<AuthResult> {
  ...
  const user = await userRepository.create({ email, displayName, passwordHash: credential.hash, passwordSalt: credential.salt, passwordParams: credential.params });
...
export async function login(email: string, password: string, device: DeviceContext): Promise<AuthResult> {
  ...
  if (recentFailures >= MAX_LOGIN_FAILURES) throw new RateLimitedError(`Too many failed login attempts. Try again in ${LOGIN_WINDOW_MINUTES} minutes.`);
  const user = await userRepository.findByEmail(normalizedEmail);
  if (!user) { await loginAttemptRepository.record(normalizedEmail, device.ipAddress, false); throw new InvalidCredentialsError('Incorrect email or password.'); }
  const valid = await verifyPassword(password, { hash: user.passwordHash, salt: user.passwordSalt, params: user.passwordParams });
  if (!valid) { await loginAttemptRepository.record(normalizedEmail, device.ipAddress, false); throw new InvalidCredentialsError('Incorrect email or password.'); }
  if (user.status !== 'active') { await loginAttemptRepository.record(normalizedEmail, device.ipAddress, false); throw new InvalidCredentialsError('This account is no longer active.'); }
...
async function createSession(userId: string, businessId: string, device: DeviceContext, authMethod: string): Promise<{ token: string; session: SessionRecord }> {
...
/** Used only by server-side onboarding flows after a user has been created atomically. */
export async function createAuthenticatedSession(userId: string, businessId: string, device: DeviceContext, authMethod = 'trial'): Promise<{ token: string; session: SessionRecord }> {
  return createSession(userId, businessId, device, authMethod);
}

export interface ValidatedSession { user: PublicUser; membership: BusinessMembershipRecord; session: SessionRecord; }
...
export async function validateSession(token: string): Promise<ValidatedSession | null> {
  ...
  if (lastSeenAge > LAST_SEEN_TOUCH_THRESHOLD_MS) void sessionRepository.touchLastSeen(session.id);
...
export function isRateLimitedError(error: unknown): error is RateLimitedError { return error instanceof RateLimitedError; }
export function isWeakPasswordError(error: unknown): error is WeakPasswordError { return error instanceof WeakPasswordError; }
export function isSessionNotFoundError(error: unknown): error is SessionNotFoundError { return error instanceof SessionNotFoundError; }
