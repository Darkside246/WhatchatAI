...
const capability: AgentCapability = {
  ...
  agentId: 'agent-1',
  description: 'Maintenance triage',
  allowedActions: ['maintenance.create_work_order'],
  forbiddenActions: ['lease.modify'],
...
const action = (overrides: Partial<ActionRequest> = {}): ActionRequest => ({
  ...
  requestedBy: { kind: 'AGENT', id: 'agent-1' }, riskLevel: 'HIGH',
  approval: { required: true, status: 'NOT_REQUIRED' }, status: 'PENDING_POLICY',
  correlationId: 'corr-1', createdAt: '2026-01-01T00:00:00.000Z', ...overrides,
});
...
describe('evaluateActionPolicy', () => {
  ...
  it('denies an unlisted action', () => {
    const result = evaluateActionPolicy(action({ type: 'payment.authorize' }), capability);
    expect(result.decision).toBe('DENY');
  });

  it('denies an explicitly forbidden action', () => {
    const result = evaluateActionPolicy(action({ type: 'lease.modify' }), { ...capability, allowedActions: ['lease.modify'] });
    expect(result.decision).toBe('DENY');
  });

  it('denies an agent acting under a different identity', () => {
    const result = evaluateActionPolicy(action({ requestedBy: { kind: 'AGENT', id: 'agent-evil' } }), capability);
    expect(result.decision).toBe('DENY');
  });

  it('denies risk above the capability maximum', () => {
    const result = evaluateActionPolicy(action({ riskLevel: 'CRITICAL' }), capability);
    expect(result.decision).toBe('DENY');
  });
