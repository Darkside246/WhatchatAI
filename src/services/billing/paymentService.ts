...
export function generateCheckoutReference(prefix = BIMPAY_PREFIX): string {
...
export function timingSafeSignatureMatches(expected: string, received: string): boolean {
  if (!expected || !received || expected.length !== received.length) return false;
  return createHmac('sha256', expected).update(received).digest('hex') === createHmac('sha256', received).update(expected).digest('hex');
}
  ...
  try {
    ...
    const subscription = await client.query<{ id: string }>(
      `INSERT INTO product_account_subscriptions (product_account_id, product_id, status, currency, amount_minor, billing_interval)
       ...
       RETURNING id`,
      [input.productAccountId, account.productId, currency, input.amountMinor, billingInterval],
    );
    ...
    const attempt = await client.query<{ id: string }>(
      `INSERT INTO payment_attempts (product_account_id, subscription_id, provider, status, currency, amount_minor, checkout_reference)
       VALUES ($1, $2, $3, 'PENDING', $4, $5, $6) RETURNING id`,
      [input.productAccountId, subscriptionId, provider, currency, input.amountMinor, reference],
    );
...
export async function verifyBiMPayTransfer(input: PaymentVerificationInput): Promise<{ paymentAttemptId: string; subscriptionId: string; productAccountId: string; status: 'ACTIVE' }> {
  ...
  try {
    await client.query('BEGIN');
    const result = await client.query<{ id: string; product_account_id: string; subscription_id: string | null; amount_minor: string; currency: string; status: string }>(
      `SELECT id, product_account_id, subscription_id, amount_minor, currency, status FROM payment_attempts WHERE checkout_reference = $1 FOR UPDATE`, [reference],
    );
    const attempt = result.rows[0];
    if (!attempt) throw new PaymentReferenceNotFoundError('Checkout reference was not found.');
    if (attempt.status === 'VERIFIED') {
      if (attempt.provider_event_id === input.providerEventId) {
        await client.query('ROLLBACK');
    ...
    if (attempt.subscription_id) {
      await client.query(`UPDATE product_account_subscriptions SET status = 'ACTIVE', current_period_start = now(), current_period_end = CASE WHEN billing_interval = 'year' THEN now() + interval '1 year' WHEN billing_interval = 'one_time' THEN NULL ELSE now() + interval '1 month' END, activated_at = now(), updated_at = now() WHERE id = $1`, [attempt.subscription_id]);
    }
...
export async function submitPaymentProof(input: { userId: string; productAccountId: string; paymentAttemptId: string; proofUrl: string; note?: string }) {
  const membership = await pool.query(`SELECT 1 FROM business_memberships bm JOIN product_accounts pa ON pa.business_id = bm.business_id WHERE pa.id = $1 AND bm.user_id = $2 AND bm.status = 'active'`, [input.productAccountId, input.userId]);
  if (membership.rowCount !== 1) throw new BillingAccountNotFoundError('Product account membership not found.');
