...
export async function listUserProductAccounts(userId: string): Promise<ProductAccountAccess[]> {
  ...
  return Promise.all(accounts.map(async (account) => ({
    account,
    entitlements: await productAccounts.listEntitlements(account.id),
    operationalAccess: account.status === 'ACTIVE',
  })));
}

/**
 * Provisions a focused product tenant for an already authenticated user.
 * Each product gets its own business and therefore its own data boundary,
 * WhatsApp connection, membership, billing and audit scope.
 */
export async function provisionProductAccount(input: {
  ...
  try {
    ...
    const product = await client.query<{ id: string; product_key: ProductKey }>(
      `SELECT id, product_key FROM product_catalog WHERE product_key = $1 AND is_active = true`,
      [input.productKey],
    );
    ...
    await client.query(
      `INSERT INTO product_account_provisioning_events (product_account_id, event_type)
       VALUES ($1, 'CREATED'), ($1, 'PROVISIONED')`,
      [accountId],
    );
    ...
    if (!account) throw new Error('Provisioned product account could not be reloaded');
    return { account, entitlements: await productAccounts.listEntitlements(account.id), operationalAccess: true };
  } catch (error) {
...
export async function getProductAccountAccess(userId: string, accountId: string): Promise<ProductAccountAccess> {
  const account = await productAccounts.findById(accountId);
  if (!account || account.ownerUserId !== userId) throw new ProductAccountNotFoundError('Product account not found.');
