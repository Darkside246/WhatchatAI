...
export class BusinessRepository {
  ...
  async setPiiWarningEnabled(id: string, enabled: boolean): Promise<BusinessRecord | null> {
    const { rows } = await this.db.query<BusinessRow>(
      `UPDATE businesses SET pii_warning_enabled = $2, updated_at = now() WHERE id = $1 RETURNING ${BUSINESS_COLUMNS}`,
      [id, enabled],
    );
    return rows[0] ? toRecord(rows[0]) : null;
  }
