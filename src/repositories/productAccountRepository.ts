...
// created_at/updated_at/expires_at are typed as `string` here, not `Date` -
// pool.ts's own doc comment establishes this as the whole codebase's
// convention: a global TIMESTAMPTZ type parser already converts every row's
// timestamp columns to a real ISO string before the driver even returns the
// row, specifically so no repository ever needs (or should call)
// .toISOString() on one. The previous `Date` typing here was wrong at
// runtime the whole time - just never exercised by a real caller with a
// non-null expires_at, or a query through listByBusiness, until tonight.
interface AccountRow {
  id: string; business_id: string; product_id: string; product_key: ProductKey; owner_user_id: string | null;
  display_name: string; status: ProductAccountStatus; created_at: string; updated_at: string;
}
interface EntitlementRow { entitlement_key: string; is_enabled: boolean; limit_value: number | null; source: ProductEntitlement['source']; expires_at: string | null; }
function toAccount(row: AccountRow) {
  return { id: row.id, businessId: row.business_id, productId: row.product_id, productKey: row.product_key, ownerUserId: row.owner_user_id, displayName: row.display_name, status: row.status, createdAt: row.created_at, updatedAt: row.updated_at };
}
...
export class ProductAccountRepository {
  ...
  async listByBusiness(businessId: string) { const { rows } = await this.db.query<AccountRow>(`${ACCOUNT_SELECT} WHERE pa.business_id = $1 ORDER BY pa.created_at ASC`, [businessId]); return rows.map(toAccount); }
